      ...
      if (snapshot.exists()) {
        snapshot.forEach((childSnapshot) => {
          ...
          if (leaf.leafIndex === leafIndex) {
            // Remove the leaf from database
            set(ref(database, `leaves/${childSnapshot.key}`), null)
            console.log(`Removed leaf with leafIndex ${leafIndex} from database`)
          }
        })
      }
    } catch (error) {
      console.error('Error removing leaf:', error)
      throw error
    }
  },
